...
def reorder_box(pts):
	...
	D = pairwise_distances(tl[np.newaxis], rightMost)[0]
...
def im_process(im_path='./midtest/img/filename.jpg'):
	im_ori = cv.imread(sys.path[0] + im_path)
	h,w = im_ori.shape[:2]
	im_resize = cv.resize(im_ori, (h//6, w//6))
	...
	lower_hsv, upper_hsv = np.array([30,49,49]), np.array([90,255,255])
	...
	im_eroded = cv.erode(mask, kernel, 5)
	im_dilated = cv.dilate(im_eroded, kernel, 1)
...
def save_list_images(list_im=[], labels=[]):
	for i in range(len(list_im)):
		filename = sys.path[0] + './img/no04_%s.png' %(labels[i])
